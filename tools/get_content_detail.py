...
class GetContentDetailTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        content_id = tool_parameters.get("content_id", "").strip()

        if not endpoint:
            yield self.create_text_message("Endpoint is required")
            return
        if not content_id:
            yield self.create_text_message("Content ID is required")
            return
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                yield self.create_text_message("Content not found. Please check the endpoint and content ID")
                return
            ...
            try:
                ...
                yield self.create_text_message(
                    f"Successfully retrieved content '{title}' (ID: {content_id}) "
